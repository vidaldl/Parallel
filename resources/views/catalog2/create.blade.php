          ...
          <form method="POST" id="formulario" action="{{route('catalog2.update', $catalog_item2s->id)}}" enctype="multipart/form-data">
            @csrf
              ...
              <div class="form-group" id="d-precio">
                ...
                <input id="button_link" type="input" name="button_link" class="form-control @error('button_link') is-invalid @enderror"  value="{{isset($catalog_item2s) ? $catalog_item2s->button_link : ''}}">
                @error('button_link')
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                  </span>
